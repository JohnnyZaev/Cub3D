...
static void	renha(t_mlx_god *god)
{
		...
		&& god->render->j >= 0 && god->render->j < god->size_y)
	{
		if (god->rays[god->render->i].hitvert
			...
			god->img.data[(int)god->render->i
				+ (int)god->render->j * god->size_x]
...
static void	renhel(t_mlx_god *god)
{
	god->render->raydist = god->rays[god->render->i].distance
		* cos(god->rays[god->render->i].angle
			- (god->player->angle * (M_PI / 180)));
	god->render->wallstrip = (TAIL_SIZE / god->render->raydist)
		* god->render->distpj;
	...
	{
		god->render->distancetop = god->render->j - god->render->top;
		god->txty = (int)((float)TAIL_SIZE / god->render->wallstrip
				* god->render->distancetop) & (TAIL_SIZE - 1);
		if (god->render->i >= 0 && god->render->i < god->size_x
			&& god->render->j >= 0 && god->render->j < god->size_y)
			god->img.data[(int)god->render->i + (int)god->render->j
				* god->size_x] = god->c_color;
		god->render->j++;
	}
}
...
void	render3d(t_mlx_god *god)
{
	...
	while (god->render->i < god->size_x)
	{
		...
		{
			god->render->distancetop = god->render->j - god->render->top;
			god->txty = (int)((float)TAIL_SIZE
					/ god->render->wallstrip * god->render->distancetop)
				& (TAIL_SIZE - 1);
			if (god->render->i >= 0 && god->render->i < god->size_x
				&& god->render->j >= 0 && god->render->j < god->size_y)
				god->img.data[(int)god->render->i + (int)god->render->j
					* god->size_x] = god->f_color;
			god->render->j++;
		}
		god->render->i++;
	}
